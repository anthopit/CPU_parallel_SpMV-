...
void MatrixVectorCSR_unrolled_4(int m, int n, const int* irp, const int* ja,const double* as, const double* x, double* restrict y)
{
    int i,j;
    double t0, t1, t2, t3;
#pragma omp parallel for shared(x,y,irp,ja,as) private(i,j,t0,t1,t2,t3) schedule(static)
    for (i = 0; i < m; i++) {
        t0 = 0.0;
        t1 = 0.0;
        t2 = 0.0;
...
//    print_csr_mtx_2D(m, n, nz, irp, ja, as);
    ...
    for (int try=0; try < ntimes; try ++ ) {
        double t1 = wtime();
        MatrixVectorCSR_unrolled_4(m, n, irp, ja, as, x, y);
        double t2 = wtime();
