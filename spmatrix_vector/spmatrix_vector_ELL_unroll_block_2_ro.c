...
const int BBS=1000;
void MatrixVector(int rows, const int* maxnz, int** ja, double** as, const double* x, double* restrict y)
{
    int row,col;
    ...
    for (row = 0; row < rows; row += BBS) {
        ...
        col     = 0;
        innerMatrixVector(nr, maxnz, &(ja[row]), &(as[row]), *maxnz, &(x[col]), 0.0, &(y[row]));
    }
...
//   print_ellpack_mtx_2D(m, n, &maxnz, ja, as);
    ...
    for (int try=0; try < ntimes; try ++ ) {
        double t1 = wtime();
        MatrixVector(m, &maxnz, ja, as, x, y);
        double t2 = wtime();
